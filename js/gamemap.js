...
function trueans() {
	...
	document.querySelector(".q_text").classList.add("active");
...
function resultread() {
	...
	let result = document.querySelector(".resultmap");
	for (let i = 0; i < 10; i++) {
		result.insertAdjacentHTML("beforeend", '<div class="resultnum">第' + (i + 1) + "問</div>");
		if (q_torf[i] == "t") {
			result.insertAdjacentHTML("beforeend", '<div class="resulttext"><div class="maru">〇</div><ruby class="rquiztext">' + quiz[i].text + "<rt>" + quiz[i].answer + "</rt></ruby><img class='mapimg' src= '" + quiz[i].src + "'></div>");
		} else {
			result.insertAdjacentHTML("beforeend", '<div class="resulttext"><div class="batu">×</div><ruby class="rquiztext">' + quiz[i].text + "<rt>" + quiz[i].answer + "</rt></ruby><img class='mapimg' src= '" + quiz[i].src + "'></div>");
		}
